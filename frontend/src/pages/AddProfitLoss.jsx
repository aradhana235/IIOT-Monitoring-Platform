...
export default function AddProfitLoss({ onClose, onSaved }) {
  const [form, setForm] = useState(INITIAL_FORM);
  const [errors, setErrors] = useState({});
  const [saving, setSaving] = useState(false);
  const [apiError, setApiError] = useState("");

  // ---- Auto formula (read-only, recalculates live) ----
  ...
  const handleSubmit = async (e) => {
    ...
    setApiError("");
    try {
      const res = await ProfitLossAPI.create(payload);
      onSaved(res.data);
    } catch (err) {
      setApiError("Could not save this entry. Please check the backend and try again.");
    } finally {
      setSaving(false);
    }
  };
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div className="pl-form-grid">
              ...
              <div className="pl-field pl-full">
                <div className="pl-formula-note">
                  ...
                  <br />
                  Loss Amount = Steam Loss × Hours/Day × Cost/kg ={" "}
                  <strong>₹{lossAmount}</strong> &nbsp;|&nbsp; Savings ({form.recoveryPercent}% recovery):{" "}
          ...
          <div className="pl-modal-footer">
            ...
            <button type="submit" className="pl-btn pl-btn-primary" disabled={saving}>
              <Plus size={16} /> {saving ? "Saving..." : "Save Entry"}
            </button>
          </div>
        </form>
      </div>
